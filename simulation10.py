...
def PV_panel_function(x, surface_area, thickness, solar_output, max_power) -> float:
    ...
    print(PV_time_expression)
    return PV_time_expression

def PV_cooling_function(x, surface_area, thickness, solar_output, max_power) -> float:
    PV_evaluation = PV_panel_function(x, surface_area, thickness, solar_output, max_power)
...
def PV_piecewise(x, surface_area, thickness, solar_output, max_power):
    PV_time_expression = Piecewise((PV_panel_function(x, surface_area, thickness, solar_output, max_power), x <= evaluate_num), (PV_cooling_function(x, surface_area, thickness, solar_output, max_power), x > evaluate_num))
    return PV_time_expression
...
def PV_new_time_expression_list(x, surface_area, thickness, solar_output):
    ...
    for item in range(0, len(max_power_list)):
        PV_new_time_exp = PV_piecewise(x, surface_area, thickness, solar_output, max_power_list[item])
...
def PV_new_delta_T_list(time_expression_list):
    PV_delta_T_list = []
    at_final_time = PV_new_time_expression_list(evaluate_num*2, surface_area, thickness, solar_output)
    at_initial_time = PV_new_time_expression_list(0, surface_area, thickness, solar_output)
...
def PV_new_total_cost_list():
    ...
    for item in max_power_list:
        new_cost = PV_calculate_system_cost(PV_panel_cost, item, PV_fixed_costs)
...
def PV_calculate_annual_savings(PV_delta_T, fraction_wood_used):
    annual_savings = fraction_wood_used * total_firewood_cost * (PV_delta_T)/final_water_temp
...
def PV_new_annual_savings_list(PV_delta_T_list):
    ...
    for item in PV_delta_T_list:
        new_annual_savings = PV_calculate_annual_savings(item, fraction_wood_used)
...
def PV_new_payback_period_list(PV_total_cost_list, PV_annual_savings_list):
    ...
    for item in range(0, len(max_power_list)):            
        new_payback = PV_calculate_payback_period(PV_total_cost_list[item], PV_annual_savings_list[item])
...
def PV_plot_time_temp_graph(surface_area, thickness, solar_output):
    ...
    for item in range(0, len(max_power_list)):
            pw = np.piecewise(x, [x < evaluate_num, x >= evaluate_num], [
                lambda z: PV_panel_function(z, surface_area, thickness, solar_output, max_power_list[item]),
...
PV_time_expression_output = PV_new_time_expression_list(x, surface_area, thickness, solar_output)
...
PV_payback_period_output = PV_new_payback_period_list(PV_total_cost_output, PV_annual_savings_output)
